...
import type { RealtimeTrainDetail } from "@/types";

defineProps<{
  open: boolean;
  detail: RealtimeTrainDetail | null;
  loading: boolean;
}>();
  ...
  <transition name="drawer-fade">
    <div v-if="open" class="drawer-backdrop" @click.self="emit('close')">
      <aside class="detail-drawer glass-card">
        ...
        <div v-if="loading" class="drawer-loading">正在加载实时详情…</div>
        <div v-else-if="detail" class="drawer-content">
          <header class="detail-hero">
            <p class="eyebrow">实时详情</p>
          ...
          <section class="detail-section">
            ...
            <div class="detail-grid">
              ...
              <div><span>区间</span><strong>{{ detail.route_signature || "--" }}</strong></div>
              <div v-if="detail.train.from_station_name || detail.train.to_station_name">
                <span>查询区间</span>
                <strong>{{ detail.train.from_station_name || "--" }} → {{ detail.train.to_station_name || "--" }}</strong>
              </div>
            </div>
          </section>
          ...
          <section class="detail-section">
            ...
            <div class="price-grid">
              <article
                v-for="seat in detail.seat_offers"
                :key="`${detail.route_signature}-${seat.seat_code}`"
                class="price-chip"
              >
                <span>{{ seat.seat_name }}</span>
                <strong>{{ seat.price !== null ? `¥${seat.price.toFixed(1)}` : "--" }}</strong>
                <small>{{ seat.inventory_text || "--" }}</small>
              </article>
              <p v-if="detail.seat_offers.length === 0" class="drawer-empty">
                当前详情没有可展示的票价或余票。
              </p>
            </div>
          </section>
        ...
        <div v-else class="drawer-empty">选择一条实时查询结果后查看详情</div>
      </aside>
    </div>
  </transition>
</template>
